...
# Authorization wrapper to check token
def token_required(func):
    ...
    wrapper.__name__ = func.__name__  # To preserve the original function name
    return wrapper
...
@bp.route('/counters/<string:counter_id>', methods=['GET'])
@token_required
def get_counter_by_id(counter_id):
    counter = supabase.table('counters').select('*').eq('counter_id', counter_id).execute().data
    if not counter:
        return jsonify({"message": "Counter not found"}), 404
    return jsonify(counter[0]), 200
...
@bp.route('/token/<string:token_id>', methods=['GET'])
@token_required
def get_token_details(token_id):
    # Fetch token details
    token_query = supabase.table('tokens').select('*').eq('token_id', token_id).execute().data
    if not token_query:
        return jsonify({"message": "Token not found"}), 404
    token = token_query[0]

    # Fetch patient details
    patient_query = supabase.table('patients').select('patient_id, name').eq('patient_id', token['patient_id']).execute().data
    if not patient_query:
        return jsonify({"message": "Patient not found"}), 404
    patient = patient_query[0]

    # Fetch counter details
    counter_query = supabase.table('counters').select('counter_id, service_type').eq('counter_id', token['counter_id']).execute().data
    if not counter_query:
        return jsonify({"message": "Counter not found"}), 404
    counter = counter_query[0]
    ...
    token_details = {
        ...
        "counter_name": f"Counter {token['counter_id']}",  # Adjust as per your naming convention
        "service_type": counter['service_type'],
        "status": token['status'],
        "token_created_at": token.get('created_at', "N/A")  # Add this field if available
    }
